...
function editarSalas(id) {
    const action = "editarSala"; // Acción que indicamos para que sea procesada en el backend
    $.ajax({
        url: 'ajax.php', // URL donde se enviará la solicitud
        type: 'GET', // Método GET
        async: true, // Realizamos la solicitud de manera asincrónica
        data: {
            editarSala: action, // Acción para identificar que es una solicitud de edición de sala
            id: id // El ID de la sala que queremos editar
        },
        success: function(response) {
            const datos = JSON.parse(response); // Asumimos que la respuesta es un JSON

            // Asignamos los datos al formulario de edición
            $('#nombre').val(datos.nombre); // Cargamos el nombre de la sala en el campo correspondiente
            $('#mesas').val(datos.mesas); // Cargamos el número de mesas en el campo correspondiente
            $('#id').val(datos.id); // Asignamos el ID de la sala al campo oculto
            $('#btnAccion').val('Modificar'); // Cambiamos el valor del botón a "Modificar"
        },
        error: function(error) {
            console.log('Error al obtener los datos de la sala:', error); // Manejamos el error si ocurre alguno
        }
    });
}
